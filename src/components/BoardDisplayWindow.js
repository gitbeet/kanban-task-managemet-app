...
export default function BoardDisplayWindow() {
  ...
  return (
    <div className="z-0 overflow-x-auto w-[100vw] min-h-full bg-neutral-700 dark:bg-primary-200 text-primary-100 dark:text-primary-500 p-4">
      {currentBoardData.length > 0 && (
        <div className="flex min-h-full  justify-start items-stretch">
          ...
          <div
            ...
            className="cursor-pointer min-w-[17rem] min-h-full top-0 bottom-0    bg-neutral-500 dark:bg-primary-300 dark:bg-opacity-50 rounded-md ml-6 mr-6"
          >
      ...
      {currentBoardData.length === 0 && (
        <>
          <div className="relative left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2  w-[22rem] flex flex-col items-center p-4 space-y-4 text-primary-100 dark:text-neutral-900">
